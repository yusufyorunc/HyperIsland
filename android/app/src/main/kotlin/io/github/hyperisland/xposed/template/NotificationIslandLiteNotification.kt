...
object NotificationIslandLiteNotification : IslandTemplate {
    ...
    private fun removeTitlePrefix(subtitle: String, cleanedTitle: String): String {
        if (cleanedTitle.isEmpty() || cleanedTitle.length > 30) return subtitle
        if (!subtitle.startsWith(cleanedTitle)) return subtitle

        var i = cleanedTitle.length
        while (i < subtitle.length && subtitle[i].isWhitespace()) i++
        if (i >= subtitle.length || (subtitle[i] != ':' && subtitle[i] != '：')) return subtitle

        i++
        while (i < subtitle.length && subtitle[i].isWhitespace()) i++
        return subtitle.substring(i)
    }

    private fun cleanSubtitle(subtitle: String, cleanedTitle: String): String {
        var s = subtitle.replace(SUBTITLE_COUNT_PREFIX, "")
        s = removeTitlePrefix(s, cleanedTitle)
        return s.trim()
    }
